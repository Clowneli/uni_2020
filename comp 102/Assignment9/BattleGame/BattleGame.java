...
public class BattleGame{
    ...
    public static final Color WINNING_CARD = new Color(204,255,0,140);
    ...
    public void restart(){
        ...
        this.compScore = 0;        // the number of points scored by the computer player
        this.playScore = 0;
        this.tableComputer = new ArrayList<Card>();  // the list of Cards that the computer has played
    ...
    public void pickupCard(){
        ...
        if ((this.deck.size() >= 0)){
            for(int i = 0; i < this.hand.length; i++){
                if (this.hand[i] == null){
                    this.hand[i] = deck.get(0);
                    this.deck.remove(0);
                }
            }
        }
        else {UI.println("Deck is Empty");}
        this.redraw();
    }
    ...
    public void drawHandCards(){
        ...
        for(int i = 0; i < this.hand.length; i++){
                if (this.hand[i] != null){
                    this.hand[i].draw(HAND_LEFT + i*CARD_SPACING, HAND_TOP);
                }
            }
    }
     ...
     *    tableComputer = new ArrayList<Card>();  // the list of Cards that the computer has played
    ...
    public void drawTableCards(){
        ...
        /*# YOUR CODE HERE */
        for(int i=0;i<this.tableComputer.size();i++){
            if(this.playLastWin == 1){
                UI.setColor(WINNING_CARD);
                UI.fillRect(TABLE_LEFT + (tableComputer.size()-1)*CARD_OVERLAP-5, TABLE_TOP_COMPUTER-5,CARD_WIDTH+10,CARD_HEIGHT+10);
            }
            this.tableComputer.get(i).draw(TABLE_LEFT + i*CARD_OVERLAP, TABLE_TOP_COMPUTER);   
        }
        for(int i=0;i<this.tablePlayer.size();i++){
            if(this.playLastWin == 2){
                UI.setColor(WINNING_CARD);
                UI.fillRect(TABLE_LEFT + (tablePlayer.size()-1)*CARD_OVERLAP-5, TABLE_TOP_PLAYER-5,CARD_WIDTH+10,CARD_HEIGHT+10);
            }
            this.tablePlayer.get(i).draw(TABLE_LEFT + i*CARD_OVERLAP, TABLE_TOP_PLAYER);
        }
    }
    ...
    public void playBattle(){
        ...
        if(this.hand[0] != null){
            ...
            if (tableComputer.get(tableComputer.size()-1).getRank() > tablePlayer.get(tablePlayer.size()-1).getRank()){
                this.compScore++;
                this.playLastWin = 1;}
            else if (tableComputer.get(tableComputer.size()-1).getRank() < tablePlayer.get(tablePlayer.size()-1).getRank()){
                this.playScore++;
                this.playLastWin = 2;
            }
            else{this.playLastWin = 3;}
            for(int i = 0; i < this.hand.length; i++){
                if (i != 0){
            ...
            this.redraw();
            if((this.playScore == this.TARGET_SCORE) || (this.compScore == this.TARGET_SCORE)){
                this.endGame();
            }
    ...
    public void replaceCard(){
        if (remainingReplaces > 0 ){
            /*# YOUR CODE HERE */
            this.deck.add(this.hand[this.selectedPos]);
            this.hand[this.selectedPos] = this.deck.get(0);
            this.deck.remove(0);            
            this.remainingReplaces--;
        }
        this.redraw();
    }
    ...
    public void moveLeft(){
        /*# YOUR CODE HERE */
        Card temp = hand[this.selectedPos-1];
        hand[this.selectedPos-1] = hand[this.selectedPos];
        hand[this.selectedPos] = temp;
