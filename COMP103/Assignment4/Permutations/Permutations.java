...
public class Permutations {
    ...
    public void extendPermutation(Set<String> remainingItems, Stack<String> permutationSoFar, List<List<String>> allPermutations){
        ...
        if((remainingItems.size()==0) || (allPermutations.size()>=10000)){
            
            allPermutations.add(permutationSoFar);
        }
        else{
            String temp = "";
            //For loop being used to get the first item of the set.
            for (String item : remainingItems){
                temp = item;
                break; //ends the foor loop after 1 instance
            }
            permutationSoFar.add(temp);
            remainingItems.remove(temp);
            extendPermutation(remainingItems, permutationSoFar, allPermutations);
            //remainingItems.add(permutationSoFar.pop()); // not working
        }
    }
    ...
    public void printAll(List<List<String>> permutations){
        ...
        for (int i=0; i<permutations.size(); i++){
            for (String str : permutations.get(i)){UI.println(str);}
            UI.println();
        }
        UI.println("----------------------");
