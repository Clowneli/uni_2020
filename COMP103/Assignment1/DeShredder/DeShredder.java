...
public class DeShredder {
    ...
    public void rotateList(){
        /*# YOUR CODE HERE */
        List<Shred> rotateShreds = new ArrayList<Shred>();
        for(int count = 1; count < allShreds.size(); count++){
            rotateShreds.add(allShreds.get(count));
        }
        rotateShreds.add(allShreds.get(0));
        allShreds = rotateShreds;
        display();
    }
    ...
    public void shuffleList(){
        /*# YOUR CODE HERE */
        List<Shred> shuffleShreds = new ArrayList<Shred>();
        for(Shred random: allShreds){
            int shuffle = shuffleShreds.size();
            shuffleShreds.add((int) (Math.random()*shuffle), random);
        }
        allShreds = shuffleShreds;
        display();
    ...
    public void doMouse(String action, double x, double y){
        ...
        if (action.equals("released")){
            ...
            /*# YOUR CODE HERE */
            if((!isFromListEmpty())&&(fromPosition<fromStrip.size())){
                if((toStrip.size()<toPosition)){toPosition = toStrip.size();}
                changePos(toStrip,toPosition);
            }
    ...
    public void changePos(List<Shred> strip,int x){
    ...
    public List<Shred> getStrip(double y){
        ...
        else if (completedStrips.size() < 1){
            return workingStrip;
        }
