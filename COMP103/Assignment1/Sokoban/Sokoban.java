...
public class Sokoban {
    ...
    private ArrayList<ActionRecord> history = new ArrayList<ActionRecord>(); //array list for past records
    private ArrayList<ActionRecord> redo = new ArrayList<ActionRecord>(); //array list for past records of history
     ...
     */
    public void doLoad() {
        history = new ArrayList<ActionRecord>(); //resets array list for new level
    ...
    /**
     * Undo button code
     * Uses actions based on the inverse of the action last used
     * drops action from list upon end of function
     */
    public void doUndo(){
        if(history.size() != 0){
            // sets variables for what to do
            int recordSize = history.size()-1;
            String direction = history.get(recordSize).direction();
            //        
            if(history.get(recordSize).isPush()){
            ...
            // drops last action from list
            redo.add(history.get(recordSize));
            history.remove(recordSize);
        }
    }
    
    /**
     * Redo button code
     * uses actions to undo the last undo
     */
    public void doRedo(){
        if(redo.size() != 0){
            // sets variables for what to do
            int recordSize = redo.size()-1;
            String direction = redo.get(recordSize).direction();
            //        
            isUndo = true;
            moveOrPush(direction);
            
            // drops last action from list
            redo.remove(recordSize);
            isUndo = false;
        }
    }
