...
public class MazeSearch {
    ...
    public MazeNode<MazeCell> firstCell;
    
    //the path 
    public ArrayList<ArrayList<MazeCell>> fullPath = new ArrayList<>();
    ...
    public boolean exploreFromCell(MazeCell cell, MazeNode<MazeCell> parentCell) {
        ...
        if (cell == maze.getGoal()) {
            ...
            MazeNode<MazeCell> neighbourCell = new MazeNode<>(cell, parentCell);
            //Adds the child cell to its parent.
            parentCell.addNeighbours(neighbourCell);
            ...
            neighbourCell.setNeighbour(neighbours);
            ...
            for (MazeCell neighbour : cell) {
                if (neighbours == 1 && cell != startingCell) {
                    ...
                    blockPaths(neighbourCell);
                    return false;
                } else {
                    exploreFromCell(neighbour, neighbourCell);
                }
            }
        }
        return false;
    ...
    public void blockPaths(MazeNode<MazeCell> neighbourCell) {
        if (neighbourCell.numberOfChildren() == 0 && neighbourCell.getNeighbour() - 1 == neighbourCell.getRecordNeighbour()) {
            //Gets the child cell and marks it red.
            neighbourCell.getCell().draw(Color.red);
            //Gets the parent cell and removes the child.
            MazeNode<MazeCell> parentCell = neighbourCell.getParent();
            parentCell.removeChild(neighbourCell);
    ...
    public void exploreFromCellAll(MazeCell cell, ArrayList<MazeCell> thePath) {
        if (stopNow) {
            return;
    ...
    public void exploreFromCellShortest(MazeCell start) {
        ...
        ArrayList<MazeCell> shortPath = fullPath.get(0); //Gets the first path in the full path list.
        pathCount = shortPath.size(); // Gets the size of the path

        for (ArrayList<MazeCell> p : fullPath) {
     ...
     * recursive.
     */
    public void find(MazeCell cell, ArrayList<MazeCell> path) {
        ...
        if (cell == maze.getGoal()) {
            ...
            for (MazeCell neighbour : cell) {
                if (neighbours > 1 || cell == startingCell) {
                    ...
                    ArrayList<MazeCell> updatedPath = new ArrayList<>(path);
    ...
    public void doMouse(String action, double x, double y) {
        if (action.equals("released")) {
            ...
            fullPath.clear(); // Clear the paths
