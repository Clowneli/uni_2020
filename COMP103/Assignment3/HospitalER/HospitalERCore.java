...
public class HospitalERCore{
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            //Treats each patient and removes from treatment room if treatment is completed
            //Adds each patients stats to the total stats
            Set<Patient> toRemove = new HashSet<Patient>();
            for (Patient patient: treatmentRoom){
                if (!(patient.noMoreTreatments() || patient.completedCurrentTreatment())){
