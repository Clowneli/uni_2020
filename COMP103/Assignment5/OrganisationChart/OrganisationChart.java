...
public class OrganisationChart {
    ...
    public void setupGUI(){
        ...
        UI.addButton("Load Chart", ()->{loadTree(UIFileChooser.open("File with a Decision Tree"));});
        UI.addButton("Save chart", this::saveOrg);
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        for (Position p : pos.getTeam()){
            Position outcome = findPosition(x,y,p);
    ...
    private void movePosition(Position pos, Position target) {
        ...
        if(!inSubtree(pos,target)){        
            if ((pos == null) || (target == null)){return;}   //invalid arguments.
            Position temp = pos;
            target.addToTeam(temp);
            temp = pos.getManager();
            temp.removeFromTeam(pos);
        }
    }
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        ...
        if (pos == subtree) {UI.println("inSubtree");return true;}
        for (Position p : pos.getTeam()){
            boolean ans = inSubtree(p,subtree);
            if (ans){return true;}
        }
        return false;
    }
    ...
    public void saveOrg(){
        ...
        String treeName = UI.askString("What is your Organisations name? ");
        String fileName = treeName+".txt";
        //Create a new file
        File file = new File(fileName);
        //Try and write the file and call print An
        try(PrintWriter print = new PrintWriter(file)){
            printAn(print,organisation);
    ...
    public void printAn(PrintWriter print, Position org){
        //If the tree isnt null
        if(org != null){
            //Get the answer and print it else get and print the question
            if(org.isManager()){
                print.println("Manager: " + org.toStringFull());
                for(Position p : org.getTeam()){
                    printAn(print,p);
                }
            }
            else{
                print.println("Worker: " + org.toStringFull());
            }
        }
    }
    /**
     * Method to load a tree
     */
    ...
    public Position loadSubTree(Queue<String> lines){
        ...
        double teamSize = line.nextDouble();
        if(line.hasNextLine()){
            line.nextLine();
        }
        Position node = new Position(role,offset);
        if (type.equals("Manager:")){
            for(int i = 0; i>teamSize;i++){    
                Position teamMember = loadSubTree(lines);
                node.addToTeam(teamMember);
            }
        }
        return node;
    }
